...
class SHNE_Encoder(nn.Module):
    def __init__(self, args, p_content, word_embed):
        ...
        self.a_latent = torch.nn.Parameter(torch.ones(
            args.A_n, args.embed_d), requires_grad=True)
        #self.p_latent = torch.nn.Parameter(torch.ones(args.P_n, args.embed_d), requires_grad=True)
        self.v_latent = torch.nn.Parameter(torch.ones(
            args.V_n, args.embed_d), requires_grad=True)
        self.rnn_p = nn.LSTM(args.embed_d, args.embed_d,
