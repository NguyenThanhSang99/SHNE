...
class input_data():
    def __init__(self, args):
        ...
        def load_p_content(path, word_n=100000):
            ...
            for i in range(len(p_content)):
                if len(p_content[i]) > self.args.c_len:
                    ...
                    p_content[i] = np.lib.pad(
                        p_content[i], (0, pad_len), 'constant', constant_values=(0, 0))
        ...
        self.p_content, self.p_content_id = load_p_content(
            path=self.args.datapath + 'content.pkl')
        self.word_embed = load_word_embed(
            path=self.args.datapath + 'word_embedding.txt')
                        ...
                        for k in range(j - window, j + window + 1):
                            if k and k < walk_L and k != j:
                                ...
                                if neighNode[0] == 'a' and random.random() < triple_sample_p[0]:
                                    ...
                                    triple = [int(centerNode[1:]), int(
                                        neighNode[1:]), int(negNode)]
